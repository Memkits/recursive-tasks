...
(deftask dev! []
  ...
  (comp
    ...
    (reload :on-jsload 'recursive-tasks.core/on-jsload
            :cljs-asset-path ".")
...
(deftask rsync []
  (with-pre-wrap fileset
    (sh "rsync" "-r" "target/" "repo.tiye.me:repo/Memkits/recursive-tasks" "--exclude" "main.out" "--delete")
    fileset))
...
(deftask watch-test []
  ...
  (comp
    (watch)
    (test :namespaces '#{recursive-tasks.test})))
